...
def main():
    ...
    height = 720
    image = np.full((height, width, 3), (0, 0, 255), dtype=np.uint8) # Создаем массив под изображение
    z_buffer = np.full((height, width), 99999.99999) # z буфер, имеет такой же размер как и наше изображение, будет показывать расстояние от камеры то дочки на сцене (не перекрытой ничем другим)
    color = (255, 255, 255) # Выбираем цвет
    light = (0, 0, 1) # Направление света от центра координат (пока что там же где и камера)

    vertices, faces, polygonsDotAmount = parseObj(objFilename)

    # Задаем сдвиг, глубину и размер для масштабирования
    x_offset = width//2
    y_offset = 0
    z_offset = 0
    size= 30
